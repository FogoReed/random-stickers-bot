...
import datetime
import sqlite3
from db_operations import get_pack_limit, count_packs, count_stickers, send_random_sticker, update_user, get_reply_chance, set_reply_chance, conn
from translations import get_translation, get_user_language, set_user_language
...
def random_pack(bot, message):
    ...
    if not send_random_sticker(bot, chat_id, reply_to_message_id=message.message_id):
        bot.reply_to(message, get_translation(get_user_language(user_id), "no_packs"))
...
def stats(bot, message):
    ...
    bot.reply_to(message, get_translation(get_user_language(user_id), "stats").format(count=count, stickers_total=stickers_total, limit=limit))
...
def set_reply_chance_command(bot, message):
    ...
    if len(args) < 2:
        bot.reply_to(message, get_translation(get_user_language(user_id), "set_reply_chance_usage"))
        return
    try:
        ...
        bot.reply_to(message, get_translation(get_user_language(user_id), "invalid_chance"))
        return

    set_reply_chance(chat_id, new_chance)
    bot.reply_to(message, get_translation(get_user_language(user_id), "reply_chance_set").format(chance=args[1]))
...
def get_reply_chance_command(bot, message):
    ...
    bot.reply_to(message, get_translation(get_user_language(user_id), "get_reply_chance").format(chance=chance))
...
def ban_pack(bot, message):
    ...
    args = message.text.split()
    if len(args) < 2:
        bot.reply_to(message, get_translation(get_user_language(user_id), "ban_pack_usage"))
        return
    pack_name = args[1]
    ...
    with conn:
        ...
        if cur.rowcount == 0:
            bot.reply_to(message, get_translation(get_user_language(user_id), "pack_not_found").format(pack_name=pack_name))
            log(f"chat_id={chat_id}: Attempt to ban non-existent pack '{pack_name}'")
        else:
            bot.reply_to(message, get_translation(get_user_language(user_id), "pack_banned").format(pack_name=pack_name))
...
def unban_pack(bot, message):
    ...
    args = message.text.split()
    if len(args) < 2:
        bot.reply_to(message, get_translation(get_user_language(user_id), "unban_pack_usage"))
        return
    pack_name = args[1]
    ...
    with conn:
        ...
        if cur.rowcount == 0:
            ...
            bot.reply_to(message, get_translation(get_user_language(user_id), "pack_unbanned").format(pack_name=pack_name))
...
def list_packs(bot, message):
    ...
    text = get_translation(get_user_language(user_id), "pack_list") + "\n"
...
def clear_packs(bot, message):
    ...
    bot.reply_to(message, get_translation(get_user_language(user_id), "packs_cleared"))
...
def set_pack_limit(bot, message):
    ...
    args = message.text.split()
    if len(args) < 2:
        bot.reply_to(message, get_translation(get_user_language(user_id), "set_pack_limit_usage"))
        return
    try:
        ...
        bot.reply_to(message, get_translation(get_user_language(user_id), "invalid_limit"))
        return
    ...
    bot.reply_to(message, get_translation(get_user_language(user_id), "pack_limit_set").format(limit=new_limit))
...
def get_pack_limit_command(bot, message):
    ...
    user_id = message.from_user.id
    limit = get_pack_limit(chat_id)
    bot.reply_to(message, get_translation(get_user_language(user_id), "get_pack_limit").format(limit=limit))
...
def help_command(bot, message):
    ...
    chat_id = message.chat.id
    user_id = message.from_user.id
    bot.reply_to(message, get_translation(get_user_language(user_id), "help"))
...
def top_users(bot, message):
    ...
    if not rows:
        bot.reply_to(message, get_translation(get_user_language(user_id), "no_users"))
        return

    text = get_translation(get_user_language(user_id), "top_users") + "\n"
    for i, (username, first_name, stickers, media) in enumerate(rows, 1):
        name = f"@{username}" if username else first_name
        text += f"{i}. {name} — 🎯 {stickers} {get_translation(get_user_language(user_id), 'stickers_label')}, 📷 {media} {get_translation(get_user_language(user_id), 'media_label')}\n"
    bot.reply_to(message, text)
...
def set_language_command(bot, message):
    ...
    keyboard.row(
        telebot.types.InlineKeyboardButton(get_translation(get_user_language(user_id), "lang_ru"), callback_data=f"lang:ru:{user_id}"),
        telebot.types.InlineKeyboardButton(get_translation(get_user_language(user_id), "lang_uk"), callback_data=f"lang:uk:{user_id}"),
        telebot.types.InlineKeyboardButton(get_translation(get_user_language(user_id), "lang_en"), callback_data=f"lang:en:{user_id}")
    )

    # Send message with buttons
    bot.send_message(chat_id, get_translation(get_user_language(user_id), "select_language"), reply_markup=keyboard)
...
def handle_language_callback(bot, call):
    ...
    if len(data) != 3 or data[0] != "lang" or data[2] != str(user_id):
        bot.answer_callback_query(call.id, get_translation(get_user_language(user_id), "invalid_callback"))
        log(f"chat_id={chat_id}, user_id={user_id}: Invalid callback or unauthorized language change attempt")
        return

    lang = data[1]
    if lang not in ["ru", "uk", "en"]:
        bot.answer_callback_query(call.id, get_translation(get_user_language(user_id), "unsupported_language"))
    ...
    log(f"chat_id={chat_id}, user_id={user_id}: Language changed to {lang}")
